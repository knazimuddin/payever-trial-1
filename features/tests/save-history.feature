Feature: Save history records
  ...
  Scenario: Payment action completed for test mode
    Given I use DB fixture "transactions/test-transaction-for-history"
    And I mock Elasticsearch method "singleIndex" with:
      """
      {
        "arguments": [
    ...
    When I publish in RabbitMQ channel "async_events_transactions_micro" message with json:
      """
      {
        "name": "payever.event.payment.action.completed.test_mode",
        "payload": {
          "payment": {
            "id": "b140034c74959b9f45c383e33f937e56",
            "uuid": "ad738281-f9f0-4db7-a4f6-670b0dff5327",
      ...
      """
    Then I process messages from RabbitMQ "async_events_transactions_micro" channel
    When look for model "TestTransaction" by following JSON and remember as "transaction":
      """
      {
        "uuid": "ad738281-f9f0-4db7-a4f6-670b0dff5327"
      }
    ...
    Then model "TestTransaction" with id "{{transaction._id}}" should contain json:
      """
      {
        "history": [
          {
